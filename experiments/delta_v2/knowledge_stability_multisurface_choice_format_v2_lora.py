...
def train_multisurface_choice_format_v2_candidate(
    *,
    config_path: Path,
    repo_root: Path,
    ...
    allowed_choice_controls = {
        "generative-sft-plus-paired-ranking-choice-format-weight4-v1":
            (1.0, 4.0, "single-verbose-answer"),
        "generative-sft-plus-paired-ranking-choice-format-multineg-v1":
            (1.0, 1.0, "residual-failure-family-v1"),
    }
    choice_control = (
        float(objective.get("choice_loss_weight", -1.0)),
        float(objective.get("choice_format_ranking_weight", -1.0)),
        str(objective.get("choice_format_negative_family", "")),
    )
    if (
    ...
    for row_id, row in rows_by_id.items():
        ...
        if row_id in choice_row_ids:
            ...
            if choice_control[2] == "residual-failure-family-v1":
                negative_texts.extend([
                    f"{gold}\n\n**Reasoning:**",
                    (
                        "To determine the correct source contract, "
            ...
            if kind == "recall":
                ...
                if unit.get("source_stream") == "replay_recall":
                    ...
                    if choice_control[1] > 0:
                        ...
                        format_terms = []
                        verbose_outputs = []
                        for negative in choice_verbose_examples[row_id]:
                            negative_log_probability, negative_output = (
                                _completion_log_probability(
                                    model, negative, torch
                                )
                            )
                            negative_mean = (
                                negative_log_probability
                                / int(negative["assistant_tokens"])
                            )
                            format_terms.append(
                                torch.nn.functional.softplus(
                                    1.0 - (gold_mean - negative_mean)
                                )
                            )
                            verbose_outputs.append(negative_output)
                        format_ranking = (
                            sum(format_terms) / len(format_terms)
                        )
                        unit_loss = (
                            unit_loss
                            + choice_control[1] * format_ranking
                        )
                        model_forwards += len(format_terms)
                        del verbose_outputs
    ...
    metrics = {
        ...
        "choice_format_negative_family": choice_control[2],
