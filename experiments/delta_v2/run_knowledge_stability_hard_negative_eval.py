...
CONFIG_SCHEMA = "delta.knowledge_stability_hard_negative_eval_config.v1"
PROTOCOL_SCHEMA = "delta.knowledge_stability_replay_eval_protocol.v1"
PROTOCOL_ID = "delta-v2-knowledge-stability-hard-negative-eval-v1"
CONDITION_ID = "d11_stability_hard_negative_eval"
RUN_ID = "delta-v2-d11-stability-hard-negative-eval-modal-v1"
TRAINING_RUN_ID = (
    "delta-v2-d11-stability-hard-negative-lora-qwen35-08b-modal-v1"
)
PROTOCOL_PATH = Path(
    "experiments/delta_v2/"
    "knowledge_stability_hard_negative_eval_protocol.yaml"
)
EVAL_PATH = Path(
...
TRIGGER = {
    ...
    "training_launch_commit": "cc81700",
    "training_receipt_sha256": "656ba2de76177713cbeb02a355684a0310f7428f5c3edee5e6619fe24aff5dbc",
    "training_metrics_sha256": "3fc85d6eaee54ff486f26444b99cab5770050993e6dd18050bb4cb42d547da45",
    "adapter_sha256": "9bd6daa1144e435062d1b052849332d71daee305bedd2a87dfe165fbec8a77f5",
}


class HardNegativeEvalError(ValueError):
    """The d11 stability pre-gate cannot run outside its frozen receipt."""
...
def validate_config(
    ...
    if (
        ...
        raise HardNegativeEvalError(
            "d11 evaluation identity changed"
        )
    ...
    if (
        ...
        raise HardNegativeEvalError("d11 protocol changed")
    ...
    if (
        ...
        raise HardNegativeEvalError("d11 evidence changed")
    ...
    if dict(dataset) != {
        ...
        raise HardNegativeEvalError("d11 dataset changed")
    if dict(_mapping(protocol.get("generation"), "generation")) != {
        ...
        raise HardNegativeEvalError("d11 generation changed")
    if dict(_mapping(protocol.get("gates"), "gates")) != {
        ...
        raise HardNegativeEvalError("d11 gates changed")
    ...
    if (
        ...
        raise HardNegativeEvalError("d11 boundary changed")
    if config.get("model") != {
        ...
        raise HardNegativeEvalError("d11 model changed")
    ...
    if (
        ...
        raise HardNegativeEvalError("d11 runtime changed")
    if config.get("eval") != {
        "module":
            "experiments.delta_v2."
            "run_knowledge_stability_hard_negative_eval",
        "path": str(EVAL_PATH),
        "sha256": dataset["sha256"],
    }:
        raise HardNegativeEvalError("d11 eval changed")
    if _relative_path(config["output"]["dir"], "output.dir") != Path("runs") / RUN_ID:
        raise HardNegativeEvalError("d11 output changed")
    if config.get("modal") != {
        "app_path": MODAL_APP_PATH,
        "gpu": "A10G",
        "timeout_seconds": 3600,
    }:
        raise HardNegativeEvalError("d11 Modal changed")
    ...
    if (
        ...
        raise HardNegativeEvalError("d11 probes changed")
