...
CONFIG_SCHEMA = "delta.knowledge_stability_multisurface_eval_config.v1"
PROTOCOL_SCHEMA = "delta.knowledge_stability_replay_eval_protocol.v1"
PROTOCOL_ID = "delta-v2-knowledge-stability-multisurface-eval-v1"
CONDITION_ID = "d9_stability_multisurface_eval"
RUN_ID = "delta-v2-d9-stability-multisurface-eval-modal-v1"
TRAINING_RUN_ID = (
    "delta-v2-d9-stability-multisurface-lora-qwen35-08b-modal-v1"
)
PROTOCOL_PATH = Path(
    "experiments/delta_v2/"
    "knowledge_stability_multisurface_eval_protocol.yaml"
)
EVAL_PATH = Path(
...
TRIGGER = {
    ...
    "training_launch_commit": "71695fb",
    "training_receipt_sha256": "9754524e590775ea1da8a9de81452a19382b592b43a29ac75e8282d9dd20af58",
    "training_metrics_sha256": "c753afac00ebed075e383bdb3aaf7afffb26e052b0ce8439e72f951a3c337f3b",
    "adapter_sha256": "e97b01b392728cf6dab404e6d500beed2e1abbf289a49a2eaac0de2b7424b938",
}


class MultisurfaceEvalError(ValueError):
    """The d9 stability pre-gate cannot run outside its frozen receipt."""
...
def validate_config(
    ...
    if (
        ...
        raise MultisurfaceEvalError("d9 evaluation identity changed")
    ...
    if (
        ...
        raise MultisurfaceEvalError("d9 protocol binding changed")
    ...
    if (
        ...
        raise MultisurfaceEvalError("d9 protocol changed")
    ...
    if (
        ...
        raise MultisurfaceEvalError("d9 evidence changed")
    ...
    if dict(dataset) != {
        ...
        raise MultisurfaceEvalError("d9 dataset changed")
    if dict(_mapping(protocol.get("generation"), "generation")) != {
        ...
        raise MultisurfaceEvalError("d9 generation changed")
    if dict(_mapping(protocol.get("gates"), "gates")) != {
        ...
        raise MultisurfaceEvalError("d9 gates changed")
    ...
    if (
        ...
        raise MultisurfaceEvalError("d9 boundary changed")
    if config.get("model") != {
        ...
        raise MultisurfaceEvalError("d9 model changed")
    ...
    if (
        ...
        raise MultisurfaceEvalError("d9 runtime changed")
    if config.get("eval") != {
        "module":
            "experiments.delta_v2.run_knowledge_stability_multisurface_eval",
        "path": str(EVAL_PATH),
        "sha256": dataset["sha256"],
    }:
        raise MultisurfaceEvalError("d9 eval changed")
    if _relative_path(config["output"]["dir"], "output.dir") != Path("runs") / RUN_ID:
        raise MultisurfaceEvalError("d9 output changed")
    if config.get("modal") != {
        "app_path": MODAL_APP_PATH,
        "gpu": "A10G",
        "timeout_seconds": 3600,
    }:
        raise MultisurfaceEvalError("d9 Modal changed")
    ...
    if (
        ...
        raise MultisurfaceEvalError("d9 probes changed")
