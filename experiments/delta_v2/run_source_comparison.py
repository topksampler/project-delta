...
RUN_RECEIPT_SCHEMA = "delta.source_comparison_run_receipt.v1"
EXPECTED_RUN_ID = "delta-v2-c3-source-comparison-qwen35-08b-modal-v1"
...
EXPECTED_MODULE = "experiments.delta_v2.run_source_comparison"
...
def validate_run_config(
    ...
    *,
    ...
    if (
        ...
        or config.get("condition_id") != "c3_source_comparison"
        or config.get("eval_environment_id") != EXPECTED_ENVIRONMENT_ID
        or config.get("task") != "eval"
    ):
        raise BaselineProtocolError("source-comparison run identity changed")
    ...
    if binding != {
        ...
        "oracle_protocol_sha256": ORACLE_PROTOCOL_SHA256,
        "request_sha256": EXPECTED_REQUEST_SHA256,
    } or protocol_path != PROTOCOL_PATH:
        raise BaselineProtocolError(
            "source-comparison protocol binding changed"
        )
    ...
    if (
        ...
        raise BaselineProtocolError(
            "source-comparison protocol bytes changed"
        )
    ...
    if config.get("model") != {
        ...
        raise BaselineProtocolError(
            "source-comparison model binding changed"
        )
    if config.get("runtime") != EXPECTED_RUNTIME:
        raise BaselineProtocolError(
            "source-comparison runtime binding changed"
        )
    ...
    if (
        ...
        raise BaselineProtocolError(
            "source-comparison truth data changed"
        )
    ...
    if (
        ...
        raise BaselineProtocolError(
            "source-comparison acceptance data changed"
        )
    if _relative_path(
        ...
        raise BaselineProtocolError(
            "source-comparison output binding changed"
        )
    if config.get("modal") != {
        ...
        raise BaselineProtocolError(
            "source-comparison Modal binding changed"
        )
    return protocol, baseline_protocol, items, evidence, feature_evidence
...
def _prepare(
    *,
    ...
    request_audit = audit_source_comparison_requests(requests)
    ...
    if (
        ...
        raise BaselineProtocolError(
            "source-comparison request bytes changed"
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run delta_v2 source-comparison representation control."
    )
