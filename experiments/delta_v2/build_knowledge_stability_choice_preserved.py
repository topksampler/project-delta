...
PROTOCOL_PATH = Path(
    ...
    "knowledge_stability_choice_preserved_data_protocol.yaml"
)
DESIGN_PATH = Path(
...
class ChoicePreservedDataError(ValueError):
    """Choice-preserved stability data failed its frozen audit."""
...
def _load_yaml(path: Path) -> Mapping[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise ChoicePreservedDataError("protocol must be a mapping")
    return value
...
def _false_card(
    ...
    if not isinstance(value, Mapping) or set(value) != set(card):
        raise ChoicePreservedDataError("false card shape changed")
    return value


def _choice_train_prompt(
    ...
    ranked = sorted(
        ...
        key=lambda option: hashlib.sha256(
            (
                "delta-v2-knowledge-choice-train-v2"
                + "\0"
                + source_id
                + "\0"
    ...
    gold_index = [
        canonical_json(option) for option in ranked
    ].index(correct)
    option_lines = [
    ...
    prompt = (
        "Select the single compact source contract that exactly matches "
        f"{_display_name(record)} in vLLM v0.26.0.\n"
        + "\n".join(option_lines)
...
def validate_protocol(
    ...
    if (
        protocol.get("schema")
        != "delta.knowledge_stability_choice_preserved_data_protocol.v1"
        or protocol.get("protocol_id")
        != "delta-v2-knowledge-stability-choice-preserved-data-v1"
        or protocol.get("dataset_id") != DATASET_ID
        or protocol.get("state") != "preregistered-for-build"
    ):
        raise ChoicePreservedDataError("data protocol identity changed")
    builder = protocol.get("builder")
    if not isinstance(builder, Mapping) or (
        builder.get("path")
        != "experiments/delta_v2/build_knowledge_stability_choice_preserved.py"
        or _sha256(repo_root / str(builder["path"]))
        != builder.get("sha256")
    ):
        raise ChoicePreservedDataError("builder binding changed")
    ...
    if not isinstance(parent, Mapping) or (
        ...
        raise ChoicePreservedDataError("parent partition changed")
    if protocol.get("construction") != {
        ...
        raise ChoicePreservedDataError("construction changed")
    if protocol.get("execution_boundary") != {
        ...
        raise ChoicePreservedDataError("data boundary changed")
...
def build(
    *, protocol_path: Path, repo_root: Path
    ...
    assignments: dict[str, dict[str, tuple[str, str]]] = {
        ...
        "family_value_swap": {},
        "full_card_swap": {},
        "annotation_boundary_swap": {
            source_id: (
                canonical_json(
                    "str"
                    if cards[source_id]["annotation"] != "str"
                    else "bool"
                ),
                "synthetic:common-type-boundary-v1",
            )
            for source_id in source_ids
        },
    }
    ...
    for source_id in source_ids:
        ...
        choice_prompt, choice_answer = _choice_train_prompt(record, card)
        rows.append(
            _row(
                source_id=source_id,
                surface="choice_preserved_select",
        ...
        family_fields = {
            ...
            "full_card_swap": "$card",
            "annotation_boundary_swap": "annotation",
        }
    ...
    if (
        ...
        != {str(None): 30, "positive": 60, "negative": 60}
    ):
        raise ChoicePreservedDataError("choice-preserved audit failed")
    ...
    summary = {
        "schema": "delta.knowledge_stability_choice_preserved_data_audit.v1",
