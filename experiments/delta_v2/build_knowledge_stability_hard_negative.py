...
PROTOCOL_PATH = Path(
    ...
    "knowledge_stability_hard_negative_data_protocol.yaml"
)
DESIGN_PATH = Path(
...
class HardNegativeDataError(ValueError):
    """Hard-negative stability data failed its frozen audit."""
...
def _load_yaml(path: Path) -> Mapping[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise HardNegativeDataError("protocol must be a mapping")
    return value
...
def _false_card(
    ...
    if not isinstance(value, Mapping) or set(value) != set(card):
        raise HardNegativeDataError("false card shape changed")
    return value
...
def validate_protocol(
    ...
    if (
        protocol.get("schema")
        != "delta.knowledge_stability_hard_negative_data_protocol.v1"
        or protocol.get("protocol_id")
        != "delta-v2-knowledge-stability-hard-negative-data-v1"
        or protocol.get("dataset_id") != DATASET_ID
        or protocol.get("state") != "preregistered-for-build"
    ):
        raise HardNegativeDataError("data protocol identity changed")
    builder = protocol.get("builder")
    if not isinstance(builder, Mapping) or (
        builder.get("path")
        != "experiments/delta_v2/build_knowledge_stability_hard_negative.py"
        or _sha256(repo_root / str(builder["path"]))
        != builder.get("sha256")
    ):
        raise HardNegativeDataError("builder binding changed")
    ...
    if not isinstance(parent, Mapping) or (
        ...
        raise HardNegativeDataError("parent partition changed")
    if protocol.get("construction") != {
        ...
        raise HardNegativeDataError("construction changed")
    if protocol.get("execution_boundary") != {
        ...
        raise HardNegativeDataError("data boundary changed")
...
def build(
    *, protocol_path: Path, repo_root: Path
    ...
    if (
        ...
        != {str(None): 15, "positive": 60, "negative": 60}
    ):
        raise HardNegativeDataError("hard-negative audit failed")
    ...
    summary = {
        "schema": "delta.knowledge_stability_hard_negative_data_audit.v1",
