...
CONFIG_SCHEMA = "delta.knowledge_stability_choice_preserved_eval_config.v1"
PROTOCOL_SCHEMA = "delta.knowledge_stability_replay_eval_protocol.v1"
PROTOCOL_ID = "delta-v2-knowledge-stability-choice-preserved-eval-v1"
CONDITION_ID = "d12_stability_choice_preserved_eval"
RUN_ID = "delta-v2-d12-stability-choice-preserved-eval-modal-v1"
TRAINING_RUN_ID = (
    "delta-v2-d12-stability-choice-preserved-lora-qwen35-08b-modal-v1"
)
PROTOCOL_PATH = Path(
    "experiments/delta_v2/"
    "knowledge_stability_choice_preserved_eval_protocol.yaml"
)
EVAL_PATH = Path(
...
TRIGGER = {
    ...
    "training_receipt_sha256": "79b590415dbf794d349836dbf71a37327fc9c8222191ffe519fe7e395548a89e",
    "training_metrics_sha256": "82c316b3554469ca80bb7c5739ca623bd1c912732773a352f4df8f72d784b2b8",
    "adapter_sha256": "b37ad4713a6ae3f8750b758d7fdd974cbe2dc82942485ad68778eba481134cb2",
}


class ChoicePreservedEvalError(ValueError):
    """The d12 stability pre-gate cannot run outside its frozen receipt."""
...
def validate_config(
    ...
    if (
        ...
        raise ChoicePreservedEvalError(
            "d12 evaluation identity changed"
        )
    ...
    if (
        ...
        raise ChoicePreservedEvalError("d12 protocol changed")
    ...
    if (
        ...
        raise ChoicePreservedEvalError("d12 evidence changed")
    ...
    if dict(dataset) != {
        ...
        raise ChoicePreservedEvalError("d12 dataset changed")
    if dict(_mapping(protocol.get("generation"), "generation")) != {
        ...
        raise ChoicePreservedEvalError("d12 generation changed")
    if dict(_mapping(protocol.get("gates"), "gates")) != {
        ...
        raise ChoicePreservedEvalError("d12 gates changed")
    ...
    if (
        ...
        raise ChoicePreservedEvalError("d12 boundary changed")
    if config.get("model") != {
        ...
        raise ChoicePreservedEvalError("d12 model changed")
    ...
    if (
        ...
        raise ChoicePreservedEvalError("d12 runtime changed")
    if config.get("eval") != {
        "module":
            "experiments.delta_v2."
            "run_knowledge_stability_choice_preserved_eval",
        "path": str(EVAL_PATH),
        "sha256": dataset["sha256"],
    }:
        raise ChoicePreservedEvalError("d12 eval changed")
    if _relative_path(config["output"]["dir"], "output.dir") != Path("runs") / RUN_ID:
        raise ChoicePreservedEvalError("d12 output changed")
    if config.get("modal") != {
        "app_path": MODAL_APP_PATH,
        "gpu": "A10G",
        "timeout_seconds": 3600,
    }:
        raise ChoicePreservedEvalError("d12 Modal changed")
    ...
    if (
        ...
        raise ChoicePreservedEvalError("d12 probes changed")
