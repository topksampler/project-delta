...
CONFIG_SCHEMA = "delta.knowledge_stability_claim_covered_eval_config.v1"
PROTOCOL_SCHEMA = "delta.knowledge_stability_replay_eval_protocol.v1"
PROTOCOL_ID = "delta-v2-knowledge-stability-claim-covered-eval-v1"
CONDITION_ID = "d8_stability_claim_covered_eval"
RUN_ID = "delta-v2-d8-stability-claim-covered-eval-modal-v1"
TRAINING_RUN_ID = (
    "delta-v2-d8-stability-claim-covered-lora-qwen35-08b-modal-v1"
)
PROTOCOL_PATH = Path(
    "experiments/delta_v2/"
    "knowledge_stability_claim_covered_eval_protocol.yaml"
)
EVAL_PATH = Path(
...
TRIGGER = {
    ...
    "training_launch_commit": "d835941",
    "training_receipt_sha256": "718b6ce1f27744e61fe4cfb047e57c5754b593701be7b3c92d452ee47310ca1f",
    "training_metrics_sha256": "ca637b0e3eba00ab71fbc20be17a7e475a1503213e27a35ce971a8394ade6aba",
    "adapter_sha256": "c813888787c3e9a827c7736219a6527639b658e853e09ac8ca7e4cbc7b468a3b",
}


class ClaimCoveredEvalError(ValueError):
    """The d8 stability pre-gate cannot run outside its frozen receipt."""
...
def validate_config(
    ...
    if (
        ...
        raise ClaimCoveredEvalError("d8 evaluation identity changed")
    ...
    if (
        ...
        raise ClaimCoveredEvalError("d8 protocol binding changed")
    ...
    if (
        ...
        raise ClaimCoveredEvalError("d8 protocol changed")
    ...
    if (
        ...
        raise ClaimCoveredEvalError("d8 evidence changed")
    ...
    if dict(dataset) != {
        ...
        raise ClaimCoveredEvalError("d8 dataset changed")
    if dict(_mapping(protocol.get("generation"), "generation")) != {
        ...
        raise ClaimCoveredEvalError("d8 generation changed")
    if dict(_mapping(protocol.get("gates"), "gates")) != {
        ...
        raise ClaimCoveredEvalError("d8 gates changed")
    ...
    if (
        ...
        raise ClaimCoveredEvalError("d8 boundary changed")
    if config.get("model") != {
        ...
        raise ClaimCoveredEvalError("d8 model changed")
    ...
    if (
        ...
        raise ClaimCoveredEvalError("d8 runtime changed")
    if config.get("eval") != {
        "module":
            "experiments.delta_v2.run_knowledge_stability_claim_covered_eval",
        "path": str(EVAL_PATH),
        "sha256": dataset["sha256"],
    }:
        raise ClaimCoveredEvalError("d8 eval changed")
    if _relative_path(config["output"]["dir"], "output.dir") != Path("runs") / RUN_ID:
        raise ClaimCoveredEvalError("d8 output changed")
    if config.get("modal") != {
        "app_path": MODAL_APP_PATH,
        "gpu": "A10G",
        "timeout_seconds": 3600,
    }:
        raise ClaimCoveredEvalError("d8 Modal changed")
    ...
    if (
        ...
        raise ClaimCoveredEvalError("d8 probes changed")
