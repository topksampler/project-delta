...
CONFIG_SCHEMA = "delta.knowledge_stability_lrhalf_eval_config.v1"
PROTOCOL_SCHEMA = "delta.knowledge_stability_replay_eval_protocol.v1"
PROTOCOL_ID = "delta-v2-knowledge-stability-lrhalf-eval-v1"
CONDITION_ID = "d7_stability_lrhalf_eval"
RUN_ID = "delta-v2-d7-stability-lrhalf-eval-modal-v1"
TRAINING_RUN_ID = "delta-v2-d7-stability-lrhalf-lora-qwen35-08b-modal-v1"
PROTOCOL_PATH = Path(
    "experiments/delta_v2/knowledge_stability_lrhalf_eval_protocol.yaml"
)
EVAL_PATH = Path(
...
TRIGGER = {
    ...
    "training_launch_commit": "609d5e7",
    "training_receipt_sha256": "4248f3917f217a03f970322df02af9c9392cb8a24c1e8ef38dd81966b701832b",
    "training_metrics_sha256": "9205fc73c1d9c5e06112c246072ac9ae140b329d78e55c84611000ac8e59f2c5",
    "adapter_sha256": "cee5c6bf022eda3856854a9de6607552d4823a2d189448787f966eb8b2a20aa4",
}


class StabilityLrhalfEvalError(ValueError):
    """The d7 stability pre-gate cannot run outside its frozen receipt."""
...
def validate_config(
    ...
    if (
        ...
        raise StabilityLrhalfEvalError("d7 stability identity changed")
    ...
    if (
        ...
        raise StabilityLrhalfEvalError("d7 protocol binding changed")
    ...
    if (
        ...
        raise StabilityLrhalfEvalError("d7 protocol changed")
    ...
    if (
        ...
        raise StabilityLrhalfEvalError("d7 evidence changed")
    ...
    if dict(dataset) != {
        ...
        raise StabilityLrhalfEvalError("d7 dataset changed")
    if dict(_mapping(protocol.get("generation"), "generation")) != {
        ...
        raise StabilityLrhalfEvalError("d7 generation changed")
    if dict(_mapping(protocol.get("gates"), "gates")) != {
        ...
        raise StabilityLrhalfEvalError("d7 gates changed")
    ...
    if (
        ...
        raise StabilityLrhalfEvalError("d7 boundary changed")
    if config.get("model") != {
        ...
        raise StabilityLrhalfEvalError("d7 model changed")
    ...
    if (
        ...
        raise StabilityLrhalfEvalError("d7 runtime changed")
    if config.get("eval") != {
        "module": "experiments.delta_v2.run_knowledge_stability_lrhalf_eval",
        "path": str(EVAL_PATH),
        "sha256": dataset["sha256"],
    }:
        raise StabilityLrhalfEvalError("d7 eval changed")
    if _relative_path(config["output"]["dir"], "output.dir") != Path("runs") / RUN_ID:
        raise StabilityLrhalfEvalError("d7 output changed")
    if config.get("modal") != {
        "app_path": MODAL_APP_PATH,
        "gpu": "A10G",
        "timeout_seconds": 3600,
    }:
        raise StabilityLrhalfEvalError("d7 Modal changed")
    ...
    if len(probes) != 60 or Counter(
        ...
        raise StabilityLrhalfEvalError("d7 probes changed")
