...
CONFIG_SCHEMA = (
    "delta.knowledge_stability_choice_preserved_rank0375_eval_config.v1"
)
PROTOCOL_SCHEMA = "delta.knowledge_stability_replay_eval_protocol.v1"
PROTOCOL_ID = (
    "delta-v2-knowledge-stability-choice-preserved-rank0375-eval-v1"
)
CONDITION_ID = "d14_stability_choice_preserved_rank0375_eval"
RUN_ID = (
    "delta-v2-d14-stability-choice-preserved-rank0375-eval-modal-v1"
)
TRAINING_RUN_ID = (
    "delta-v2-d14-stability-choice-preserved-rank0375-lora-"
    "qwen35-08b-modal-v1"
)
PROTOCOL_PATH = Path(
    "experiments/delta_v2/"
    "knowledge_stability_choice_preserved_rank0375_eval_protocol.yaml"
)
EVAL_PATH = Path(
...
TRIGGER = {
    ...
    "training_launch_commit": "a5305d7",
    "training_receipt_sha256":
        "a3876decc4d9f44026fdc3ccf8492569ffcd3251a51c86a674f6c3d8232ced36",
    "training_metrics_sha256":
        "350c1445ef629021246dd37fc1ddf0bd8f74759476443ad5edb12ffa3a7d023d",
    "adapter_sha256":
        "6d2a7e97839edd5ce1292810ef8c413172d54dfc0eb9990958b6310f02e29e2a",
}


class ChoicePreservedRank0375EvalError(ValueError):
    """The d14 stability pre-gate cannot run outside its frozen receipt."""
...
def validate_config(
    ...
    if (
        ...
        raise ChoicePreservedRank0375EvalError(
            "d14 evaluation identity changed"
        )
    ...
    if (
        ...
        raise ChoicePreservedRank0375EvalError("d14 protocol changed")
    ...
    if (
        ...
        raise ChoicePreservedRank0375EvalError("d14 evidence changed")
    ...
    if dict(dataset) != {
        ...
        raise ChoicePreservedRank0375EvalError("d14 dataset changed")
    if dict(_mapping(protocol.get("generation"), "generation")) != {
        ...
        raise ChoicePreservedRank0375EvalError("d14 generation changed")
    if dict(_mapping(protocol.get("gates"), "gates")) != {
        ...
        raise ChoicePreservedRank0375EvalError("d14 gates changed")
    ...
    if (
        ...
        raise ChoicePreservedRank0375EvalError("d14 boundary changed")
    if config.get("model") != {
        ...
        raise ChoicePreservedRank0375EvalError("d14 model changed")
    ...
    if (
        ...
        raise ChoicePreservedRank0375EvalError("d14 runtime changed")
    if config.get("eval") != {
        "module":
            "experiments.delta_v2."
            "run_knowledge_stability_choice_preserved_rank0375_eval",
        "path": str(EVAL_PATH),
        "sha256": dataset["sha256"],
    }:
        raise ChoicePreservedRank0375EvalError("d14 eval changed")
    if _relative_path(config["output"]["dir"], "output.dir") != Path("runs") / RUN_ID:
        raise ChoicePreservedRank0375EvalError("d14 output changed")
    if config.get("modal") != {
        "app_path": MODAL_APP_PATH,
        "gpu": "A10G",
        "timeout_seconds": 3600,
    }:
        raise ChoicePreservedRank0375EvalError("d14 Modal changed")
    ...
    if (
        ...
        raise ChoicePreservedRank0375EvalError("d14 probes changed")
